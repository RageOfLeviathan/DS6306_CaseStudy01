...
```
\
The above plot shows a map of the United States the color of each state showing how many breweries are in each state. Below, the five states the highest brewery count are pulled out for easy reference.
\
```{r, echo=FALSE, message = FALSE, warning=FALSE}
st_data %>% slice_max(n,n=5) %>% summarise(State,n)
```
\
The states with the fewest breweries have also been pulled out. There are 8 total because of four-way tie for fith fewest breweries.
\
```{r, echo=FALSE, message = FALSE, warning=FALSE}
st_data %>% slice_min(n,n=5) %>% summarise(State,n)
```
\
Base on the visual evidence there is not an even distribution of breweries among the states. Starting with the highest ranking state (Colorado) and moving down the list, the below plot shows how which states the data under anyalysis is coming from.
\
```{r, echo=FALSE, message = FALSE, warning=FALSE}
nst = nrow(st_data)
nbrew = sum(st_data$n)
...
```
\
Below the states with the number of beers with IBU scores for each state are shown. States with below 15 ratings are grayed out to highlight them as states with a low amount of data. Another way to view this chart is as the states the are the primary influecers of any further analysis that uses IBU score. 
\
```{r, echo=FALSE, message = FALSE, warning=FALSE}
